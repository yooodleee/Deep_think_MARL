...
void UTF8String::append(int ch) {
    ...
    // when it's allocated, it's writable
...
UTF8String UTF8String::substr(int pos, int count) {
    ...
    for (int i = 0; i < count && it != end && it.firstByte(); ++it)
        ++it;
...
int UTF8String::iterator::operator*() {
    ...
    switch (codeLength(ch)) {
        ...
        case 4:
            if ((ch == 0xF0 && p[1] < 0x90)
                || (ch == 0xF4 && p[1] > 0x8F))
                throw runtime_error("invalud UTF8 character");
...
UTF8String::iterator &UTF8String::iterator::operator--() // prefix
{
    ...
    if (p + codeLength(*p) != q)
        throw runtime_error("invalud UTF8 sequence");
...
bool UTF8String::iterator::iterator::operator!=(UTF8String::iterator it) {
    return p != it.p;
}
