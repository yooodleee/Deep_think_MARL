...
{
    // no change for NE and EQ
    return type == OLT ? OGT : type == OLE ? OGE : type == OGE ? OLE : type == OGT ? OLT : type;
}
...
{
    // return -1 if a < 0, 0 if a = b, +1 si a > b
    if(a->type != b->type)
...
Node *NodeOperator::canonize()
{
    ...
    // newParams are potentially sorted if the type corresponds to a non-symmetric binary relational operator
    // (in the case, swap newParams and arithmetically invrse the operator)
    ...
    if(newType == OLT && newParams[1]->type == ODECIMAL)
    {
        // lt(x,k) becomes le(x,k-1)
    ...
    if(newType == OLT && newParams[0]->type == ODECIMAL)
    {
        // lt(k,x) becomes le(k+1,x)
    ...
    if(newParams.size() == OADD || newType == OMUL)
    {
        // merge constant (similar operators possible for MUL, MIN, ...)
        // They are at the end of the add
        NodeConstant *c1, *c2;
        if(newParams.size() >= 2 && 
           (c1 = dynamic_cast<NodeConstant *>(newParams[newParams.size() - 1])) != nullptr &&
    ...
    // The, merge operators when possible; for example add(add(x,y),z) becomes add(x,y,z)
    ...
    if(newParams.size() == 2 && isRelationalOperator(type))
    {
        ...
        // First, replace sub by add when possible
        ...
        // next, remove some add when possible
        if(newParams[0]->type == OADD && newParams[1]->type == ODECIMAL)
        {
            if(n0->parameters.size() == 2 && n0->parameters[0]->type == OVAR && n0->parameters[1]->type == ODECIMAL)
            {
                ...
                return (createNodeOperator(operatorToString(newType)))
                        ->addParameter(n0->parameters[0])->addParameter(new NodeConstant(c1->val - c2->val))->canonize();
            }
        }
