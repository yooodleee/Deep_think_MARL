...
# confirm you should author solution files
def confirmGenerate():
    print("WARNING: this cation will overwrite any solution files.")
...
ERROR_HINT_MAP = {
    ...
    You noticed that your project threw an IndexError on q1.
    ...
    You noticed that project throw an AttributeError on q3.
    While many things may cause this, it may have been from assuming
    a certain size or structure to the state space. For example, if
    you have a line of code assuming that the state is (x, y) and you
    run your code on a state with (x, y, z), this error could be thrown.
    Try making your code more general and submit again!
    """
    }
}
...
def printTest(testName, moduleDict, printTestCase=False, display=None):
    import testParser
    import testClasses
    ...
    testCase = testClasses(question, testDict)
...
def getDisplay(graphicsByDefault, options=None):
    ...
    if options is not None and options.nbGraphics:
        graphics = False
    if graphics:
        try:
