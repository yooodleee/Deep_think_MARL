...
if _Windows:
    ...
    _canvas_tfonts = ['times', 'lucidasans-25']
...
def begin_graphics(
    ...
    # Save the canvas size params
    _canvas_xs, _canvas_ys = width - 1, height - 1
    _canvas_x, _canvas_y = 0, _canvas_ys
    _bg_color = color

    # Create the roow window
    ...
    # Beind to key-down and key-up events
...
def draw_background():
    corners = [
        (0, 0), (0, _canvas_ys), (_canvas_xs, _canvas_ys), (_canvas_xs, 0)
    ]
...
def end_graphics():
    ...
    try:
        try:
            ...
            print("Ending graphics raised an execption: ", e)
    finally:
        _root_window = None
        _canvas = None
        _mouse_enabled = 0
...
def square(pos, r, color, filled=1, behind=0):
    ...
    return polygon(coords, color, color, filled, behind=behind)
...
# You bind to key-down and key-up events.


_keysdown = {}
_keyswaiting = {}
# This holds an unprocessed key release. You delay key releases by up to
# one call to keys_pressed() to get round a prob with auto repeat.
_got_release = None
