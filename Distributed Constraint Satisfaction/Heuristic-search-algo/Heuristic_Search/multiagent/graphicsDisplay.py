...
GHOST_COLORS.append(formatColor(9, .3, .9))         # Blue
...
class InfoPane:
    ...
    def toScreen(self, pos, y=None):
        """Translates a point relative from the bottom left of
        the info pane."""
...
class PacmanGraphics:
    ...
    def swapImages(self, agentIndex, newState):
        """Changes an image from a ghost to a pacman or vis verse
        (for capture)."""
    ...
    def movePacman(self, position, direction, image):
        ...
        endpoints = self.getEndpoints(position)
    ...
    def animatePacman(self, pacman, prevPacman, image):
        if self.frameTime < 0:
            print('Pree any key to step forward, "q" to play')
            keys = wait_for_keys()
            if 'q' in keys:
                self.frameTime = 0.1
            ...
            for yNum, cell in enumerate(x):
                if cell:    # There's a wall here
                    ...
                    if (not nIsWall) and (not eIsWall):
                        # inner circle
                        circle(
                            screen2, WALL_RADIUS * self.gridSize,
                    ...
                    if (not nIsWall) and (not wIsWall):
                        ...
                        circle(
                            screen2, WALL_RADIUS * self.gridSize,
                            wallColor, wallColor, (90, 180), 'arc'
                        )
                    ...
                    if (sIsWall) and (eIsWall) and (not seIsWall):
                        ...
                        line(
                            add(screen, (self.gridSize * WALL_RADIUS - 1, self.gridSize * (1) * WALL_RADIUS)),
                            add(screen, (self.gridSize * 0.5, self.gridSize * (1) * WALL_RADIUS)),
                            wallColor
                        )
                        line(
    ...
    def drawExpandedCells(self, cells):
        """Draws an overlay of expanded grid positions for search 
        agents."""
    ...
    def updateDistributions(self, distributions):
        ...
        # copy all distributions so you don't change their state
...
# =======================
# Note: to make an animated gif from this pstscript output, try the command:
# convert -delay 7 -compress lzw -layers optimize frame* out.gif
