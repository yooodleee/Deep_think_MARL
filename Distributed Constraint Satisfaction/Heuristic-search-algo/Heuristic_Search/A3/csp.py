...
class Constraint:
    ...
    it has been satisfied, so each type of constraint must be a different
    class. For example a constraint of notEquals(V1, V2) must be a dufferent
    class from a constraint of greaterThan(V1, V2), as they must implement
    checks of satisfaction.
    
    However one can define a class of general table constraints, as below,
    then can capture many different constraints.
    
    On initialization the constraint's name can be given as well as the 
    constraint's scope. IMPORTANT, the scope is ordered! E.g., the constraint
    greaterThan(V1, V2) is not the same as the constraint greaterThan(V2, V1).
    """

    def __init__(self, name, scope):
        """Create a constraint obj, specify the constraint name (a string
        ) and its scope (an ORDERED list of variable obj)."""
    ...
    def __str__(self):
        return "Cnstr_{}({})".format(self.name(), list(map(lambda var: var.name(), self.scope())))
    
    def printConstraint(self):
        print(
            "Cons: {} Vars = {}".format(self.name(), [v.name() for v in self.scope()])
        )


# obj for holding a constraint prob
class CSP:
    """CSP class groups together a set of variables and a set of constraints to
    form a CSP prob. Provides a useful place to put some other functions that
    depend on which variables and constraints are active"""

    def __init__(
            self, name, variables, constraints
    ):
        """Create a CSP prob obj passing it a name a list
        variables obj and a list of constraint obj"""
        ...
        for v in variables:
            if v not in varsInCnst:
                print(
                    "Warning: variable {} is not in any constraint of the CSP {}"
        ...
        for v in varsInCnst:
            if v not in variables:
                print(
                    "Error: variable {} appears in constraint but specified as one of the variables of the CSP {}"
                    .format(v.name(), self.name())
                )
    ...
    def constraintsOf(self, var):
        ...
        try:
            ...
            print(
                "Error: tried to find constraint of variable {} that isn't in this CSP {}"
    ...
    def check(self, solutions):
        ...
        if these satisfy all the constraints. Return list of erroneous
        solutions."""
