...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "b",
        help="The board number to solve",
        type=int,
    )
    parser.add_argument(
        "-a",
    ...
    parser.add_argument(
        "-e",
        "--gacEnforce",
        help="Don't use search only apply gacEnforce",
        action="store_true",
    )
    parser.add_argument(
        "-m",
        "--model",
        help="Choose CSP model/binary not equals or alldiff",
        choices=['neq', 'alldiff'],
        default='neq',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=['fixed', 'random', 'mv'],
        default='fixed',
    )
    args = parser.parse_args()

    if args.b < 1 or args.b > len(boards):
        print("{} is invalid board number. I only know about boards {}".format(args.b, 1, len(boards)))
        print("If you want to add new boards add them to the list \"boards\"")
        exit(1)
    
    ib = boards[args.b - 1]
    print("=" * 66)
    print("Solving boards {}".format(args.b))
    for row in ib:
        print(row)
    print("-" * 60)
