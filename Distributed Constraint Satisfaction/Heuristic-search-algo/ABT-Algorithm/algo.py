...
class Agent:
    ...
    def handle_no_good(self, neighbor, no_good):
        ...
        for no_good_neighbor in no_good:
            if not any(
                no_good_neighbor[0] != neighbor["name"]
                for neighbor in self.neighbor_list
            ):
                self.neighbor_list.append(
                    {
                        "name": no_good_neighbor[0],
                        "value": no_good_neighbor[1],
                        "priority": no_good_neighbor[2]
                    }
                )
        old_value = copy.deepcopy(self.value)
        self.check_local_view()
    ...
    def backtrack(self):
        for domain in self.domains:
            ...
            self.value = self.domains[0]
            self.domains.append(old_value)
            # check consistency with higher priority agent
...
agent1, agnet2, agent3 = Agent()
agent2 = Agent("red", [agent1], 2, ["red", "blue", "green"])
agent3 = Agent("red", [agent2], 3, ["red", "blie", "green"])
